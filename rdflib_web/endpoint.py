...
def setup(state):
    ...
    state.app.jinja_env.globals["python_version"] = "%d.%d.%d" % (
...
# HEAD is done by flask via GET
@endpoint.route("/graph-store", methods=["GET", "PUT", "POST", "DELETE"])
def graph_store_indirect():
    return graph_store_do(None)


# HEAD is done by flask via GET
@endpoint.route("/graph-store/<path:path>",
...
def __create_generic_endpoint():
    current_app.config["generic"] = generic_endpoint.GenericEndpoint(
        ds=current_app.config["graph"], coin_url=lambda: url_for(
            "graph_store_direct", path=str(
                rdflib.BNode()), _external=True))
...
def __end(response):
    ...
    if response.response and response.content_type.startswith(
            "text/html") and response.status_code == 200:
        response.response[0] = response.response[0].replace(
            '__EXECUTION_TIME__', "%.3f" % diff)
